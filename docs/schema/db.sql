...
CREATE TABLE IF NOT EXISTS "ApiTokenProfiles" (
    ...
    "ProfileKey" character varying(32),
    "ExpiresInDays" integer,
    "Active" boolean DEFAULT true NOT NULL,
    "CreatedAt" timestamp with time zone DEFAULT now(),
    "UpdatedAt" timestamp with time zone DEFAULT now(),
    CONSTRAINT "ApiTokenProfiles_pkey" PRIMARY KEY (id),
    CONSTRAINT "ApiTokenProfiles_Name_key" UNIQUE ("Name"),
    CONSTRAINT "ApiTokenProfiles_ProfileKey_key" UNIQUE ("ProfileKey"),
    CONSTRAINT chk_apitokenprofiles_scope CHECK ((("Scope"::text) = ANY (ARRAY['read-only'::text, 'write'::text]))),
    CONSTRAINT chk_apitokenprofiles_expires CHECK ((("ExpiresInDays" IS NULL) OR ("ExpiresInDays" > 0)))
);
CREATE INDEX IF NOT EXISTS idx_apitokenprofiles_active ON "ApiTokenProfiles" USING btree ("Active");
COMMENT ON TABLE "ApiTokenProfiles" IS '[core]';
...
CREATE TABLE IF NOT EXISTS "CliAuthSessions" (
    id integer GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    "DeviceCodeHash" character varying(64) NOT NULL,
    "UserCodeHash" character varying(64) NOT NULL,
    ...
    CONSTRAINT "CliAuthSessions_pkey" PRIMARY KEY (id),
    ...
    CONSTRAINT chk_cliauthsessions_status CHECK ((("Status"::text) = ANY (ARRAY['pending'::text, 'approved'::text, 'completed'::text, 'denied'::text, 'expired'::text])))
);
CREATE INDEX IF NOT EXISTS idx_cliauthsessions_expires ON "CliAuthSessions" USING btree ("ExpiresAt") WHERE ("ExpiresAt" IS NOT NULL);
CREATE INDEX IF NOT EXISTS idx_cliauthsessions_status ON "CliAuthSessions" USING btree ("Status");
COMMENT ON TABLE "CliAuthSessions" IS '[core]';
