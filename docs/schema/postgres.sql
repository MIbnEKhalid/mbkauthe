...
CREATE INDEX IF NOT EXISTS idx_mbkcore_users_is_local_only ON mbkcore_users USING btree (is_local_only);
...
CREATE TABLE IF NOT EXISTS mbkcore_session (
    sid character varying DEFAULT gen_random_uuid()::text NOT NULL,
    id text GENERATED ALWAYS AS (sid) STORED,
    sess jsonb DEFAULT '{}'::jsonb NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_mbkcore_session_username ON mbkcore_session USING btree (username);
CREATE INDEX IF NOT EXISTS idx_mbkcore_session_device_id ON mbkcore_session USING btree (device_id);
