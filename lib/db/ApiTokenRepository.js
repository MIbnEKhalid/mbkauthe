...
const withScope = (row) => {
  ...
  return {
    ...
    allowed_apps: perms.allowed_apps ?? null,
  };
};
...
export class ApiTokenRepository extends BaseRepository {
  ...
  async countForUser(username) {
    const { rows } = await this.executeRaw({
      ...
      text: 'SELECT COUNT(*) as token_count FROM api_tokens WHERE username = $1',
      values: [username],
    });
  ...
  async insert(username, name, token_hash, prefix, permissions, expires_at) {
    const { rows } = await this.executeRaw({
      ...
      text: `INSERT INTO api_tokens (username, name, token_hash, prefix, permissions, expires_at)
  ...
  async updateLastUsedByHash(token_hash) {
    return this.executeRaw({
      ...
      text: 'UPDATE api_tokens SET last_used = NOW() WHERE token_hash = $1',
      values: [token_hash],
    });
  }
  ...
  async findInfoById(id) {
    const { rows } = await this.executeRaw({
      ...
      text: 'SELECT username, name FROM api_tokens WHERE id = $1',
      values: [id],
    });
  ...
  async deleteById(id) {
    return this.withTransaction(async (txRepo) => {
      ...
      return txRepo.executeRaw({
        ...
        text: 'DELETE FROM api_tokens WHERE id = $1',
        values: [id],
      });
    });
  }
  ...
  async deleteByIds(ids) {
    ...
    return this.withTransaction(async (txRepo) => {
      ...
      return txRepo.executeRaw({
        ...
        text: 'DELETE FROM api_tokens WHERE id = ANY($1)',
        values: [tokenIds],
      });
    });
  }
  ...
  async deleteByIdAndUsername(id, username) {
    return this.withTransaction(async (txRepo) => {
      await txRepo._purgeDeviceCodes(
        '"apiTokenId" IN (SELECT id FROM api_tokens WHERE id = $1 AND username = $2)',
        [id, username],
      );
      return txRepo.executeRaw({
        name: "api-tokens-delete-owned",
        text: 'DELETE FROM api_tokens WHERE id = $1 AND username = $2 RETURNING name',
        values: [id, username],
      });
    });
  }
  ...
  async deleteAllByUsername(username) {
    return this.withTransaction(async (txRepo) => {
      await txRepo._purgeDeviceCodes(
        '"apiTokenId" IN (SELECT id FROM api_tokens WHERE username = $1)',
        [username],
      );
      return txRepo.executeRaw({
        name: "api-tokens-delete-all-user",
        text: 'DELETE FROM api_tokens WHERE username = $1',
        values: [username],
      });
    });
  }
