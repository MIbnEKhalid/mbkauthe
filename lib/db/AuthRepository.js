...
export class AuthRepository extends BaseRepository {
  buildSessionUserSelect({ includeProfile = false, includeTwoFA = false } = {}) {
    const fields = [
      `s.id as sid`, `s.expires_at`, `u."UserName"`, `u."UserId"`,
      `u."Active"`, `u."Role"`, `u."AllowedApps"`
    ];
    if (includeProfile) fields.push(`u."FullName"`, `u."Image"`);
    if (includeTwoFA) fields.push(`tfa."TwoFAStatus"`);
    return fields.join(", ");
  }
  ...
  async deleteAppSessionById(sessionId, queryName = "invalidate-app-session") {
    return this.executeRaw({ name: queryName, text: `DELETE FROM "Sessions" WHERE id = $1`, values: [sessionId] });
  }

  async deleteSessionBySid(sessionId, queryName = "login-delete-old-session-before-regen") {
    return this.executeRaw({ name: queryName, text: `DELETE FROM "session" WHERE sid = $1`, values: [sessionId] });
  }
  ...
  async touchTrustedDevice(deviceTokenHash, username) {
    if (this.dialect.name === "sqlite") {
      const updated = await this.executeRaw({
        ...
        text: `UPDATE "TrustedDevices" SET "LastUsed" = CURRENT_TIMESTAMP
               WHERE "DeviceToken" = $1 AND "UserName" = $2 AND "ExpiresAt" > CURRENT_TIMESTAMP
      ...
      return userRow ? { ...row, ...userRow } : null;
    }
    ...
    `;
    const result = await this.executeRaw({ name: "check-trusted-device", text: query, values: [deviceTokenHash, username] });
    return result.rows?.[0] || null;
  }
  ...
  async updateLastLoginReturnProfile(username) {
    const result = await this.executeRaw({
      ...
      text: `UPDATE "Users" SET "last_login" = NOW() WHERE "UserName" = $1 RETURNING "FullName", "Image"`,
      values: [username]
    });
    return result.rows?.[0] || null;
  }
  ...
  async getUserProfileByUsername(username, queryName = "login-get-fullname-and-image") {
    const result = await this.executeRaw({ name: queryName, text: `SELECT "FullName", "Image" FROM "Users" WHERE "UserName" = $1 LIMIT 1`, values: [username] });
    return result.rows?.[0] || null;
  }
  ...
  async insertTrustedDevice({ username, deviceTokenHash, deviceName, userAgent, ipAddress, expiresAt }) {
    return this.executeRaw({
      ...
      text: `INSERT INTO "TrustedDevices" ("UserName", "DeviceToken", "DeviceName", "UserAgent", "IpAddress", "ExpiresAt") VALUES ($1, $2, $3, $4, $5, $6)`,
  ...
  async getTwoFASecret(username) {
    const result = await this.executeRaw({ name: "verify-2fa-secret", text: `SELECT tfa."TwoFASecret" FROM "TwoFA" tfa WHERE tfa."UserName" = $1`, values: [username] });
    return result.rows?.[0] || null;
  }

  async deleteSessionsByIds(ids, queryName = "delete-sessions-by-ids") {
    if (!Array.isArray(ids) || ids.length === 0) return 0;
    const result = await this.executeRaw({ name: queryName, text: `DELETE FROM "Sessions" WHERE id = ANY($1)`, values: [ids] });
    return result.rowCount || 0;
  }
  ...
  async updateApiTokenLastUsed(tokenId, queryName = null, minIntervalMinutes = 15) {
    const query = this.dialect.name === "sqlite"
      ? `UPDATE "ApiTokens" SET "LastUsed" = CURRENT_TIMESTAMP WHERE id = ? AND ("LastUsed" IS NULL OR "LastUsed" < datetime('now', '-' || ? || ' minutes'))`
      : `UPDATE "ApiTokens" SET "LastUsed" = NOW() WHERE id = $1 AND ("LastUsed" IS NULL OR "LastUsed" < NOW() - ($2::int * INTERVAL '1 minute'))`;
    const values = [tokenId, minIntervalMinutes];
    return this.executeRaw(queryName ? { name: queryName, text: query, values } : { text: query, values });
  }

  async getSessionAuthData(sessionId, queryName = "validate-app-session") {
    const query = `SELECT s.expires_at, u."Active", u."Role", u."AllowedApps", u."UserName" FROM "Sessions" s JOIN "Users" u ON s."UserName" = u."UserName" WHERE s.id = $1 LIMIT 1`;
  ...
  async getUserFullNameByUsername(username, queryName = "get-fullname-by-username") {
    const result = await this.executeRaw({ name: queryName, text: `SELECT "FullName" FROM "Users" WHERE "UserName" = $1 LIMIT 1`, values: [username] });
    return result.rows?.[0] || null;
  }

  async getUserImageByUsername(username, queryName = "get-user-profile-pic") {
    const result = await this.executeRaw({ name: queryName, text: `SELECT "Image" FROM "Users" WHERE "UserName" = $1 LIMIT 1`, values: [username] });
    return result.rows?.[0] || null;
  }
  ...
  async deleteAllAppSessions(queryName = "delete-all-app-sessions") {
    return this.executeRaw({ name: queryName, text: `DELETE FROM "Sessions"`, values: [] });
  }

  async deleteActiveSessionStoreRows(queryName = "delete-active-session-store-rows") {
    return this.executeRaw({ name: queryName, text: `DELETE FROM "session" WHERE expire > NOW()`, values: [] });
  }
}