...
export class BaseRepository {
  constructor(adapterOrOptions = {}) {
    if (adapterOrOptions && typeof adapterOrOptions.query === "function") {
      ...
      this.dialect = adapterOrOptions?.dialect || (this.db && this.db.dialect) || postgresDialect;
    }
  }
  ...
  setDb(db, dialect = null) {
    ...
    else if (db?.dialect) this.dialect = db.dialect;
  }
  ...
  async execute(queryOrTextOrName, valuesOrQuery = [], executorOrName = this.db, name) {
    // 1. mbkauthe style: execute(name, { text, values })
    ...
    // 2. Object config style: execute({ text, values, name })
    ...
    // 3. SQL string style: execute(sqlText, values, executorOrName, name)
    ...
    if (stmtName) {
      return executor.query({ text: queryOrTextOrName, values, name: stmtName });
    }
    return executor.query(queryOrTextOrName, values);
  }
