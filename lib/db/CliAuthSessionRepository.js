...
export class CliAuthSessionRepository extends BaseRepository {
  ...
  async create({ device_code_hash, user_code_hash, client_name, profile_id, expires_at }) {
    const { rows } = await this.executeRaw({
      ...
      text: `INSERT INTO cli_auth_sessions (device_code_hash, user_code_hash, client_name, profile_id, expires_at)
             VALUES ($1, $2, $3, $4, $5)
  ...
  async findByDeviceCodeHash(device_code_hash) {
    const { rows } = await this.executeRaw({
      ...
      text: `SELECT ${SESSION_COLUMNS} FROM cli_auth_sessions WHERE device_code_hash = $1`,
  ...
  async findByUserCodeHash(user_code_hash) {
    const { rows } = await this.executeRaw({
      ...
      text: `SELECT ${SESSION_COLUMNS} FROM cli_auth_sessions WHERE user_code_hash = $1`,
  ...
  async markDenied(id) {
    return this.executeRaw({
      name: "cli-auth-deny",
      text: `UPDATE cli_auth_sessions SET status = 'denied' WHERE id = $1 AND status = 'pending'`,
      values: [id],
    });
  }
  ...
  async markExpired(id) {
    return this.executeRaw({
      ...
      text: `UPDATE cli_auth_sessions SET status = 'expired' WHERE id = $1 AND status = 'pending'`,
      values: [id],
    });
  }
  ...
  async expireStale(now = new Date()) {
    return this.executeRaw({
      ...
      text: `UPDATE cli_auth_sessions SET status = 'expired'
  ...
  async completeDelivery(id) {
    const { rows } = await this.executeRaw({
      ...
      text: `UPDATE cli_auth_sessions SET status = 'completed', pending_token = NULL
  ...
  async deleteById(id) {
    return this.executeRaw({
      ...
      text: 'DELETE FROM cli_auth_sessions WHERE id = $1',
      values: [id],
    });
  }
  ...
  async getProfileById(profile_id) {
    const { rows } = await this.executeRaw({
      ...
      text: `SELECT ${PROFILE_COLUMNS} FROM api_token_profiles WHERE id = $1`,
      values: [profile_id],
    });
  ...
  async getActiveProfileById(profile_id) {
    const { rows } = await this.executeRaw({
      ...
      text: `SELECT ${PROFILE_COLUMNS} FROM api_token_profiles WHERE id = $1 AND is_active = TRUE`,
      values: [profile_id],
    });
    return rows[0] ? normalizeProfileRow(rows[0]) : null;
  }
  ...
  async getActiveProfileByKey(profile_key) {
    const { rows } = await this.executeRaw({
      ...
      text: `SELECT ${PROFILE_COLUMNS} FROM api_token_profiles WHERE profile_key = $1 AND is_active = TRUE`,
      values: [profile_key],
    });
    return rows[0] ? normalizeProfileRow(rows[0]) : null;
  }
