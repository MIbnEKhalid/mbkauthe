...
const DEFAULT_JSON_COLUMNS = [
  ...
  "social_accounts", "meta", "conversation_history", "allowed_record_types",
  "sections", "semester"
];

const DEFAULT_BOOLEAN_COLUMNS = [];
...
const DEFAULT_TIMESTAMP_COLUMNS = [
  ...
  "resetTokenExpires", "lastResetAttempt", "last_reset_attempt",
  "deadline", "completed_at", "start_date"
];
...
function runQuery(db, rawText, rawValues, normalizeRow) {
  ...
  // DDL statements run directly via db.exec()
 ...
 * Provides unified query({ text, values, name }) and query(text, values) contracts,
 * transactionMutex management, type coercion, and row normalization.
 */
export class SqliteAdapter {
  constructor(filePathOrDb, options = {}) {
    ...
    const jsonCols = new Set([...DEFAULT_JSON_COLUMNS, ...(options.jsonColumns || [])]);
    const boolCols = new Set([...DEFAULT_BOOLEAN_COLUMNS, ...(options.booleanColumns || [])]);
    const timeCols = new Set([...DEFAULT_TIMESTAMP_COLUMNS, ...(options.timestampColumns || [])]);

    this._normalizeRow = createRowNormalizer({
      jsonColumns: jsonCols,
      booleanColumns: boolCols,
      timestampColumns: timeCols
    });
  }
...
// Backward compatibility alias: SqlitePool is identical to SqliteAdapter
