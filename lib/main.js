...
// CLI / device-flow login is attached to the main router by default so hosts do
// not need to mount it themselves. Disable with mbkautheVar.CLI_AUTH_ENABLED=false.
// The router stays mounted even when disabled; cliAuthRouter itself rejects
// requests with a "CLI auth is disabled" error (see lib/routes/cliAuth.js).
