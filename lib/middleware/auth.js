...
async function validateTokenAuthentication(req) {
  ...
  const permissions = row.permissions || { scope: 'read-only', allowed_apps: null };
  const allowedApps = (permissions.allowed_apps !== null && permissions.allowed_apps !== undefined)
    ? permissions.allowed_apps
    : row.user_allowed_apps;
  ...
  return {
    ...
    user_allowed_apps: row.user_allowed_apps,
    is_active: row.is_active,
    token_scope: permissions.scope || 'read-only'
  };
}
...
function attachApiTokenUser(req, res, tokenUser) {
  const user = {
    ...
    allowed_apps: tokenUser.allowed_apps,
    token_scope: tokenUser.token_scope || null,
  };
    ...
    try {
      ...
      if (tokenUser && !tokenUser.error) {
        ...
        if (tokenUser.role !== "superadmin") {
          const { allowed_apps, user_allowed_apps } = tokenUser;
          if (!Array.isArray(allowed_apps) || allowed_apps.length === 0) {
            return res.status(401).json(createErrorResponse(401, ErrorCodes.APP_NOT_AUTHORIZED));
          }

          const hasWildcard = allowed_apps.includes('*');
          const hasSpecific = allowed_apps.some((app) => app?.toLowerCase() === mbkautheVar.APP_NAME);

          if (hasWildcard) {
            const userHasApp = Array.isArray(user_allowed_apps) && user_allowed_apps.some((app) => app?.toLowerCase() === mbkautheVar.APP_NAME);
        ...
        if (tokenUser.token_scope && !canAccessMethod(tokenUser.token_scope, req.method)) {
          return res.status(403).json(createErrorResponse(403, ErrorCodes.TOKEN_SCOPE_INSUFFICIENT, {
            message: `Token scope '${tokenUser.token_scope}' does not allow ${req.method} requests`,
            tokenScope: tokenUser.token_scope,
