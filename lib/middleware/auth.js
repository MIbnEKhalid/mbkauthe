...
import { defaultRoleRegistry, GlobalPermissions, hasPermission, resolvePermission } from "../permissions.js";
...
function parseTokenPermissionList(raw) {
  let list = [];
  if (Array.isArray(raw)) {
    list = raw;
  } else if (raw && typeof raw === 'object') {
    list = Array.isArray(raw.permissions) ? raw.permissions : [];
...
async function validateCookieSession(req, res, next, { prefersJson }) {
  ...
  try {
    const { session_id, username } = req.session.user;
