...
import { mbkautheVar, hashApiToken, canAccessMethod } from "#config.js";
...
async function validateTokenAuthentication(req) {
  ...
  const allowedApps = permissions.allowedApps !== null ? permissions.allowedApps : row.user_allowed_apps;
  ...
  return {
    ...
    allowedApps,
    userAllowedApps: row.user_allowed_apps,
    active: row.Active,
    tokenScope: permissions.scope || 'read-only'
  };
}
...
async function validateCookieSession(req, res, next, { prefersJson }) {
  ...
  try {
    ...
    const sessionRow = await authRepo.getSessionAuthData(sessionId, prefersJson ? 'validate-app-session-for-api' : 'validate-app-session');
    ...
    if (!hasAppAccess(sessionRow.Role, sessionRow.AllowedApps)) {
      ...
      return respondSessionFailure(req, res, {
        ...
        pagename: "Home", page: `/${mbkautheVar.loginRedirectURL}`
      });
    }
    ...
    try {
      ...
      if (tokenUser && !tokenUser.error) {
        ...
        if (tokenUser.role !== "SuperAdmin") {
          const { allowedApps, userAllowedApps } = tokenUser;
          ...
          const hasSpecific = allowedApps.some((app) => app?.toLowerCase() === mbkautheVar.APP_NAME);

          if (hasWildcard) {
            const userHasApp = Array.isArray(userAllowedApps) && userAllowedApps.some((app) => app?.toLowerCase() === mbkautheVar.APP_NAME);
            if (!userHasApp) return res.status(401).json(createErrorResponse(401, ErrorCodes.APP_NOT_AUTHORIZED));
          } else if (!hasSpecific) {
            return res.status(401).json(createErrorResponse(401, ErrorCodes.APP_NOT_AUTHORIZED));
          }
        }
        ...
        if (tokenUser.tokenScope && !canAccessMethod(tokenUser.tokenScope, req.method)) {
          return res.status(403).json(createErrorResponse(403, ErrorCodes.TOKEN_SCOPE_INSUFFICIENT, {
            message: `Token scope '${tokenUser.tokenScope}' does not allow ${req.method} requests`,
...
async function reloadSessionUser(req, res) {
  ...
  try {
    ...
    const row = await authRepo.getSessionWithUserForReload(String(sessionId), 'reload-session-user');
    ...
    } else if (typeof req.cookies?.fullName === 'string') {
...
const checkRolePermission = (requiredRoles, notAllowed) => async (req, res, next) => {
  try {
    ...
    if (authUser.role === "SuperAdmin" || userRole === "SuperAdmin") return next();

    const homeRedirect = `/${mbkautheVar.loginRedirectURL}`;
    ...
    if (notAllowed && userRole === notAllowed) {
      ...
      return renderError(res, req, {
        code: 403, error: "Access Denied", message: "You are not allowed to access this resource",
        pagename: "Home", page: homeRedirect
      });
    }

    const rolesArray = Array.isArray(requiredRoles) ? requiredRoles : [requiredRoles];
    if (rolesArray.includes("Any") || rolesArray.includes("any") || rolesArray.includes("*") || rolesArray.includes(userRole)) {
      return next();
    }

    if (isJsonRequest(req)) return res.status(403).json(createErrorResponse(403, ErrorCodes.INSUFFICIENT_PERMISSIONS));
    return renderError(res, req, {
      code: 403, error: "Access Denied", message: "You do not have permission to access this resource",
      pagename: "Home", page: homeRedirect
    });
  } catch (err) {
