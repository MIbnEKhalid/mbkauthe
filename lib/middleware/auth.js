  ...
  try {
    ...
    if (sessionRow.expires_at) {
      ...
      if (!Number.isNaN(expiresMs) && expiresMs <= Date.now()) {
        console.log(`[mbkauthe] Session invalidated (expired) for user "${sessionRow.UserName || req.session.user.username}"`);
        return respondSessionFailure(req, res, {
          prefersJson,
          code: 401,
          errorCode: ErrorCodes.SESSION_EXPIRED,
    ...
    if (!sessionRow.Active) {
      console.log(`[mbkauthe] Account is inactive for user "${sessionRow.UserName || req.session.user.username}"`);
      return respondSessionFailure(req, res, {
        prefersJson,
        code: 401,
