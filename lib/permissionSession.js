 ...
 */
export async function attachSessionPermissions(sessionUser, username, knownRole = null) {
  if (!username) return resetSessionUserPermissions(sessionUser);

  try {
    const snapshot = await permissionRepository.computeEffectiveForUser(username, knownRole || sessionUser?.role);
    if (sessionUser) {
