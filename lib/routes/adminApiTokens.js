...
router.delete("/api/admin/api-tokens/bulk", sessRole("superadmin"), async (req, res) => {
  try {
    ...
    if (typeof raw_ids === "string") raw_ids = raw_ids.split(",").map((s) => s.trim()).filter(Boolean);
    const ids = (Array.isArray(raw_ids) ? raw_ids : []).map((id) => parseInt(id, 10)).filter((id) => Number.isInteger(id) && id > 0);
