...
import { sessRole, sessPerm } from "../middleware/auth.js";
...
async function loadGrantablePermissions(user) {
  ...
  const grantable = active.filter((perm) => hasPermission({ permissions: effective }, perm.permission));
...
router.get("/user/api-tokens", sessRole("any"), async (req, res) => {
  try {
    ...
    try {
      const { grantable } = await loadGrantablePermissions(req.session.user);
