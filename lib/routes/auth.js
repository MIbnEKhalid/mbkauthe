...
export async function completeLoginProcess(req, res, user, redirect_url = null, trust_device = false, method = null) {
  try {
    ...
    const inserted = await authRepo.createAppSessionWithPruning({
      username,
      expiresAt,
      meta,
    ...
    await attachSessionPermissions(req.session.user, username, user.role);
...
router.post("/api/verify-2fa", TwoFALimit, csrfProtection, async (req, res) => {
  ...
  const { username, role, user_id, allowed_apps, full_name, image } = req.session.pre_auth_user;
