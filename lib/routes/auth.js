...
export async function completeLoginProcess(req, res, user, redirect_url = null, trust_device = false, method = null) {
  try {
    ...
    if (profileRow) {
      const { full_name, image } = profileRow;
      ...
      try {
        ...
        const { full_name, image } = profileResult || {};
        if (full_name) req.session.user.full_name = full_name;
        if (image?.trim()) loginProfileImage = image;
      } catch (profileErr) {
    ...
    await attachSessionPermissions(req.session.user, username);
...
router.post("/api/login", LoginLimit, async (req, res) => {
  ...
  try {
    ...
    const { password_hash, username: auth_username, is_active, role, allowed_apps, user_id, is_enabled } = user;
    ...
    const user_for_session = { user_id: user_id || undefined, username: auth_username, role, allowed_apps };
...
router.post("/api/logout-account", LoginLimit, async (req, res) => {
  ...
  try {
    await authRepo.deleteAppSessionById(target_session_id, "logout-single-account");
    removeAccountFromCookie(req, res, target_session_id);

    const isCurrent = req.session?.user?.session_id === target_session_id;
    if (isCurrent) {
      if (req.sessionID) await authRepo.deleteSessionBySid(req.sessionID, "logout-single-account-current-sid");
      clearSessionCookies(res);
      req.session.destroy(() => {});
    }
