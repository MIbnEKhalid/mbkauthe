...
export async function completeLoginProcess(req, res, user, redirect_url = null, trust_device = false, method = null) {
  try {
    ...
    // Compute + cache the user's effective permissions on the session. This runs
    // only at login (never on the per-request authorization path) and is
    // best-effort/non-fatal so existing sessions and role checks are unaffected.
