...
const buildVerificationUrl = (req, userCode) => `${getBaseUrl(req)}/mbkauthe/cli/device/${userCode}`;
...
router.get("/mbkauthe/cli/device/:userCode", sessRole("any"), async (req, res) => {
  try {
    ...
    const profile = session.ProfileId ? await cliAuthSessionRepository.getProfileById(session.ProfileId) : null;
