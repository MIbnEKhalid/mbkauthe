...
// When CLI auth is disabled (CLI_AUTH_ENABLED=false), the router stays mounted
// but every request is rejected with an error instead of returning 404.
router.use((req, res, next) => {
  if (mbkautheVar.CLI_AUTH_ENABLED === "false") {
    return res.status(403).json({ success: false, message: "CLI auth is disabled" });
  }
  next();
});
