...
import { intersectPermissions } from "../permissions.js";
...
router.post("/api/cli/device/approve", deviceApproveLimit, sessRole("any"), async (req, res) => {
  try {
    ...
    // A profile defines the maximum permission set; the approving user can only
    // ever receive a token capped at their own effective permissions. SuperAdmins
    // hold every permission and keep the profile set as-is.
    ...
    if (role !== "superadmin" && profilePermissions.length > 0) {
      ...
      tokenPermissions = intersectPermissions(effective, profilePermissions).allows;
    }
