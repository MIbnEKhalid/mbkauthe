...
const createOAuthStrategy = async (provider, profile, done) => {
  try {
    ...
    if (provider === 'GitHub') {
      userData.github_id = user.github_id;
      userData.github_username = user.github_username;
      userData.installation_id = user.installation_id || null;
      userData.installation_target_type = user.installation_target_type || null;
    } else {
      userData.google_id = user.google_id;
      userData.google_email = user.google_email;
    }
...
const finishProviderLogin = async (req, res, provider, user) => {
  ...
  if (trustedDeviceUser && String(mbkautheVar.MBKAUTH_TWO_FA_ENABLE || "").toLowerCase() === "true" && user.two_fa_status) {
  ...
  if (String(mbkautheVar.MBKAUTH_TWO_FA_ENABLE || "").toLowerCase() === "true" && user.two_fa_status) {
