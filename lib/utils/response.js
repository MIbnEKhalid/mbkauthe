...
export function isJsonRequest(req) {
    ...
    if (url.endsWith('.json') || path.endsWith('.json')) return true;
    if ((headers['x-requested-with'] || '').toLowerCase() === 'xmlhttprequest') return true;
    if (contentType.includes('application/json')) return true;
    if (accept.includes('application/json') || (accept.includes('json') && !accept.includes('text/html'))) return true;
...
export function sendError(res, errorInput, options = {}) {
    ...
    if (typeof errorInput === 'string') {
        ...
        if (!rawDetails && process.env.NODE_ENV !== 'production') {
            rawDetails = errorInput.stack;
        }
...
export function createErrorHandler(options = {}) {
    const {
        appName = mbkautheVar?.APP_NAME || 'mbktech',
        defaultPage = '/',
    ...
    return (err, req, res, next) => {
        ...
        if (!isClientError) {
            console.error(`[${appName}] Unhandled error:`, err?.stack || err);
        }
