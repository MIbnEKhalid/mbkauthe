(() => {
  ...
  const getCookieDomains = () => {
    ...
    if (window.mbkautheConfig?.cookieDomain) domains.add(window.mbkautheConfig.cookieDomain);
    domains.add(hostname);
    if (hostname.includes('.')) domains.add(`.${hostname}`);
    return [...domains];
  };
  ...
  async function logout({ confirmLogout = true } = {}) {
    if (confirmLogout && !confirm('Are you sure you want to logout?')) return false;
  ...
  function checkSession() {
    return fetch('/mbkauthe/api/checkSession', { credentials: 'include', cache: 'no-store' })
      .then(async (response) => {
