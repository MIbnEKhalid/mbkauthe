...
import { buildEffectivePermissions, GLOBAL_APP_KEY, defaultRoleRegistry, RoleRegistry } from "../../core/permissions/roleRegistry.js";
import type { CatalogPermission, RoleDefinition, UserPermissionOverride, UserRoleAssignment, EffectivePermissionsResult, DeclaredPermission } from "../../core/types/permission.types.js";

const GLOBAL_PERMISSION: DeclaredPermission & { appKey: string } = {
...
const toBool = (val: unknown): boolean => Boolean(val === 1 || val === true || val === "1" || val === "true");
...
export class PermissionRepository extends BaseRepository {
  ...
  async ensureGlobalPermission(): Promise<void> {
    await this.upsertCatalogPermission(GLOBAL_PERMISSION);
  }

  // ---------------------------------------------------------------- Catalog

  async upsertCatalogPermission({ appKey, serviceKey, actionKey, label = null }: UpsertCatalogOptions): Promise<any> {
  ...
  async deactivateAllCatalogPermissionsForApp(appKey: string): Promise<{ rowCount: number }> {
  ...
  async syncCatalogForApp(appKey: string, declared: DeclaredPermission[] = []): Promise<void> {
    await this.withTransaction(async (txRepo) => {
      const pRepo = txRepo as unknown as PermissionRepository;
      await pRepo.deactivateAllCatalogPermissionsForApp(appKey);
      for (const perm of declared) {
        await pRepo.upsertCatalogPermission({
          appKey,
  ...
  async listCatalog(): Promise<CatalogPermission[]> {
    const result = await this.executeRaw({
  ...
  async listActiveCatalog(): Promise<CatalogPermission[]> {
    const catalog = await this.listCatalog();
    return catalog.filter((row) => row.is_active);
  }

  async listCatalogByApp(appKey: string): Promise<CatalogPermission[]> {
    const result = await this.executeRaw({
  ...
  async lookupCatalogPermission(permission: string): Promise<{ permission: string; is_active: boolean } | null> {
  ...
  async isCatalogPermissionActive(permission: string): Promise<boolean> {
  ...
  async getRoleByName(name: string): Promise<RoleDefinition | null> {
    ...
    return {
      ...
      permissions: (permsResult.rows || []).map((p: any) => p.permission),
      created_at: row.created_at,
      updated_at: row.updated_at,
    };
  }

  async createRole(name: string, permissions: string[] = [], options: RoleCreateOptions = {}): Promise<RoleDefinition | null> {
    const cleanName = normalizePermission(name);
    ...
    await this.withTransaction(async (txRepo) => {
      ...
      for (const perm of cleanPerms) {
        await txRepo.executeRaw({
          ...
          text: txRepo.dialect?.name === "sqlite"
  ...
  async contributeRolePermissions(roleName: string, permissions: string[] = [], options: RoleCreateOptions = {}): Promise<void> {
    const cleanName = normalizePermission(roleName);
    const cleanPerms: string[] = Array.from(new Set(permissions.map(normalizePermission).filter(Boolean)));

    await this.withTransaction(async (txRepo) => {
      const isSqlite = txRepo.dialect?.name === "sqlite";
      await txRepo.executeRaw({
  ...
  async loadAllRolesIntoRegistry(registry: RoleRegistry = defaultRoleRegistry): Promise<RoleRegistry> {
    try {
      const permsResult = await this.executeRaw({
  ...
  async listOverridesForUser(username: string): Promise<UserPermissionOverride[]> {
    const result = await this.executeRaw({
  ...
  async setOverride(username: string, permission: string, effect: "allow" | "deny" | string, grantedBy: string | null = null): Promise<any> {
  ...
  async removeOverride(username: string, permission: string): Promise<any> {
    return this.executeRaw({
  ...
  async clearOverridesForUser(username: string): Promise<any> {
    return this.executeRaw({
  ...
  async replaceOverridesForUser(username: string, overrides: Array<{ permission: string; effect: string }> = [], grantedBy: string | null = null): Promise<void> {
    await this.withTransaction(async (txRepo) => {
      const pRepo = txRepo as unknown as PermissionRepository;
      await pRepo.clearOverridesForUser(username);
      for (const item of overrides) {
        ...
        if (perm && ["allow", "deny"].includes(effect)) {
          await pRepo.setOverride(username, perm, effect, grantedBy);
        }
      }
      await pRepo.bumpPermissionVersion(username);
    });
  }

  // ---------------------------------------------------- User Role Assignment

  async getUserRole(username: string): Promise<UserRoleAssignment> {
  ...
  async setUserRoles(username: string, roles: string[] | string = []): Promise<void> {
  ...
  async bumpPermissionVersion(username: string): Promise<any> {
    const isSqlite = this.dialectName === "sqlite";
    return this.executeRaw({
