...
const normalizeStringFilter = (value: unknown) => (typeof value === "string" ? value.trim() : "");

const parseSuccessFilter = (value: unknown) => {
  if (value === true || value === "true") return true;
  if (value === false || value === "false") return false;
  return null;
};

const getRawQueryLog = () => (typeof getQueryLog === "function" ? getQueryLog() : (typeof (dblogin as any).getQueryLog === "function" ? (dblogin as any).getQueryLog() : []));

const filterQueryLog = (queryLog: any[], filters: any) => {
...
const sortQueryLogNewestFirst = (queryLog: any[]) =>
...
const buildSummary = (queryLog: any[]) => {
  ...
  const repeatedGroupsMap = new Map<string, any>();
...
const buildResponsePayload = (req: express.Request) => {
  const queryCount = typeof getQueryCount === "function" ? getQueryCount() : (typeof (dblogin as any).getQueryCount === "function" ? (dblogin as any).getQueryCount() : 0);
  ...
  const queryLog = sortQueryLogNewestFirst(filtered).slice(0, queryLimit);
...
router.post(["/db/reset"], LogLimit, async (req, res) => {
  try {
    ...
    else if (typeof (dblogin as any).resetQueryCount === "function") (dblogin as any).resetQueryCount();

    if (typeof resetQueryLog === "function") resetQueryLog();
    else if (typeof (dblogin as any).resetQueryLog === "function") (dblogin as any).resetQueryLog();
