 ...
 */
export function isSafeFetchUrl(urlString?: string | null): boolean {
  ...
  try {
    ...
    return allowedProtocols.includes(parsed.protocol) && !parsed.username && !parsed.password && Boolean(parsed.hostname) && !isBlockedHostname(parsed.hostname);
  } catch {
    return false;
  }
}
