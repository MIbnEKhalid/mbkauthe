...
async function deviceLogin() {
  ...
  if (!profileRef) {
    ...
    console.error("       or MBKCLI_PROFILE_ID as a fallback.");
    process.exit(1);
  }
  ...
  console.log("\n=== ACTION REQUIRED ===");
  console.log(`Open this URL in a browser and approve (code: ${user_code}):`);
  console.log(`\n    ${verification_url}\n`);
  openBrowser(verification_url);

  const pollEvery = interval || 5;
  const maxAttempts = Math.floor((expires_in || 900) / pollEvery) + 2;
